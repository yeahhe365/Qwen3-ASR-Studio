...
一个功能丰富的网页应用，旨在为 Qwen、豆包等 ASR 模型提供一个强大、高效且用户友好的操作界面。无论您是需要转录会议记录、整理语音笔记，还是进行任何形式的语音转文本工作，本工具都能为您提供流畅的体验。
...
- **高效的转录核心**:
    - **多厂商 ASR 驱动**: 支持 Qwen 官方 ASR API 与豆包语音识别极速版，并为后续厂商扩展保留 provider 结构。
...
- **ASR 接口**: Qwen 官方 OpenAI 兼容接口（`qwen3-asr-flash`）、豆包语音识别极速版（`volc.bigasr.auc_turbo`）
- **客户端技术**:
